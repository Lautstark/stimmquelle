...
import { labelOf } from './list.js';
/* One table, two renderers. `svelte/VoicePicker.svelte` says the same words as
   this file because it reads the same object — conventions.md §6.8 asks the two
   versions for the same `WORDS`, and a second copy of it would compile, pass
   both suites and drift on the first correction made to whichever file somebody
   had open. `picker-words.ts` carries the reasoning for each string. */
import {
  factsOf, language, languagesIn, speaks, WORDS,
  type Pickable, type PickerLang, type Words,
} from './picker-words.js';

/* The three names that were already this module's public door, still opening
   off it. A consumer importing `Pickable` or `factsOf` from
   `@lautstark/stimmquelle/voice-picker` is importing what it always was. */
export { factsOf, type Pickable, type PickerLang };
