...
var voices_default = {
  ...
  voices: [
    ...
    },
    {
      ...
      browser: "ok",
      proof: "spike",
      note: "A corpus, not a person: 236 speakers in one model and no name a picker can show. Listened to rather than assumed. All 236 speakers were rendered saying the same sentence on 2026-08-22, only sid changed, and none of them sounds like German - not a pacing problem and not a pitch problem, they do not sound like the language. Kerstin was rendered through the same harness as a control and came out fine, so the phoneme path was not the fault. That closes this as a route to a German female voice: it is not the closest thing to one, it is not one at all. 82 of the 236 measure in the female pitch band, which is how thin a measure pitch is. Also every speaker is slower than Kerstin - 2.11s at best against her 1.34s, at length_scale 1 for both - because MLS is LibriVox audiobook reading. Kept in the file rather than removed so the next person does not re-derive this from the speaker count.",
      speakers_auditioned: "236 of 236, 2026-08-22 - none usable for German"
    },
    {
    ...
    },
    {
      ...
      browser: "quality",
      proof: "container",
      note: "Free to ship and fine in a container - she is mitreden's container default and is in vorlaut's. Published as low only, so no other file helps. Reading each model's own phoneme_id_map instead of vits-web's fixed table is what would bring her back, and it means owning the phonemizer glue rather than calling a library. Note the 16 kHz: vorlaut's device wants exactly that, so a low voice would need no resampling at all. The cause is now diagnosed and fixed in code: the phonemizer writes the ich-Laut decomposed and this model's map has only the precomposed form, so remapPhonemeIds composes it and every id lands in range. `browser` stays `quality` until somebody has actually heard it \u2014 the proof field exists so this list cannot say tested and mean assumed. The harness has reported: she speaks, 63 ids, nothing dropped, the ich-Laut at 40 where native piper leaves a bare plosive at 16. Native piper drops the combining mark her map lacks, so it says Ik, m\xF6kte, nikt - which means native is not the oracle here and cannot flip this either way. What is left is a human ear. Heard 2026-08-22: the ich-Laut is right and she is intelligible throughout, so the remap does what it claimed. Reopened 2026-08-23 and not yet closed: the reason CONTRACT.md gave for composing - that id 40 is the form she was trained on - turned out not to follow, because the 130-entry table is generic and carries the same 'c-cedilla' at 40 under English and French (conformance/phoneme-tables.mjs). A five-sentence blind test at the config's own scales, three renders a side and labels shuffled per sentence, preferred native's bare plosive five times out of five - the listener tracked the side across a label flip, so it is the sound and not the position. A further blind ranking of every plausible symbol in her table (c-cedilla 40, x 36, c 16, and esh 96 as a deliberately wrong control) put c at 16 first and correctly picked the control out as worst, so there is no better symbol to compose onto either. Six preferences for native across three test designs, one listener - who then recognised the difference unprompted in a recording captured from the live product, which is where the browser-versus-container question finally landed: the container ran native piper and got the bare plosive, the browser composed. Resolved in 2.7.0 by removing the composition, so this path now produces native's ids exactly. PIPELINE_VERSION is 3. The table lookup that makes low voices speak at all is untouched. The verdict on the voice itself was okay but not great, which is a judgement about a 2021 low-tier model and not about the fix. `browser` stays `quality` for ever - that column is what vits-web can do, and it still cannot phonemise her. Compared against de_DE-thorsten-medium through the identical chain on 2026-08-22 and judged the worse of the two, which settles a question that had been put the other way round: a browser recording sounding worse than a container one was the container defaulting to her, not the browser chain damaging anything.",
